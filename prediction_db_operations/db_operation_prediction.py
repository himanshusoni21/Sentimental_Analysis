...
class DB_Operations_Prediction:
    def __init__(self):
        self.client = pymongo.MongoClient('mongodb://127.0.0.1:27017')
        self.path = 'Training_Database'
        self.good_file_path = 'Prediction_Raw_Validated_File/Good_Raw'
        self.bad_file_path = 'Prediction_Raw_Validated_File/Bad_Raw'
        self.FileFromDB = 'PredictionFileFrom_DB'
        self.logger = App_Logger()

    def create_db_connection(self,database_name):
        file = open('Prediction_Logs/General_Log.txt', 'a+')
        self.logger.log(file,'Entered create_db_connection() method of DB_Operation class of Prediction_db_operations package')
        file.close()
    ...
    def create_collection(self,db_object):
        file = open('Prediction_Logs/General_Log.txt', 'a+')
        self.logger.log(file,'Entered create_collection() method of DB_Operation class of Prediction_db_operations package')
        file.close()

        try:
            collection_list = db_object.collection_names()
            collection_name = 'GoodRawData'
            file = open("Prediction_Logs/CreateCollectionLog.txt", 'a+')
    ...
    def insertion_GoodRawData_into_collection(self,collection_object):
        file = open('Prediction_Logs/General_Log.txt', 'a+')
        self.logger.log(file, 'Entered insertionGoodData_into_collection() method of DB_Operation class of Prediction_db_operations package')
        file.close()

        try:
            only_files = [file for file in os.listdir(self.good_file_path)]
            file = open("Prediction_Logs/DataBaseSelectionLog.txt", 'a+')
            for f in only_files:
    ...
    def selectDataFromCollection_into_csv(self, collection_object):
        file = open('Prediction_Logs/General_Log.txt', 'a+')
        self.logger.log(file,'Entered selectDataFromCollection_into_csv() method of DB_Operation class of Prediction_db_operations package')
        file.close()
        ...
        try:
            ...
            file = open("Prediction_Logs/DataBase_Into_CSVLog.txt", 'a+')
